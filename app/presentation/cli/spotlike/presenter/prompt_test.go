...
func TestRunPromptWithMask(t *testing.T) {
	origPu := Pu
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := RunPromptWithMask(tt.args.label, tt.args.mask)
			if (err != nil) != tt.wantErr {
				t.Errorf("RunPromptWithMask() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("RunPromptWithMask() = %v, want %v", got, tt.want)
			}
		})
	}
}
