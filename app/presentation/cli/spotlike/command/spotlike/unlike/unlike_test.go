...
func TestNewUnlikeCommand(t *testing.T) {
	output := ""
	exit := o.Exit
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := NewUnlikeCommand(tt.args.exit, tt.args.cobra, tt.args.authCmd, tt.args.output)
			if got == nil {
				t.Errorf("NewUnlikeCommand() = %v, want %v", got, tt.want)
			} else {
				cmd := &c.Command{}
				cmd.SetContext(context.Background())
				if err := got.RunE(cmd, []string{}); err != nil {
					t.Errorf("Failed to run unlike command: %v", err)
				}
			}
		})
	}
...
func Test_runUnlike(t *testing.T) {
	output := ""
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := runUnlike(tt.args.output); (err != nil) != tt.wantErr {
				t.Errorf("runUnlike() error = %v, wantErr %v", err, tt.wantErr)
			}
			if *tt.args.output != tt.wantOutput {
				t.Errorf("runUnlike() output = %v, want %v", *tt.args.output, tt.wantOutput)
			}
		})
	}
}
