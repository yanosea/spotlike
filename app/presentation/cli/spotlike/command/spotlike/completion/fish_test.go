...
func TestNewCompletionFishCommand(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := NewCompletionFishCommand(tt.args.cobra, tt.args.output)
			if got == nil {
				t.Errorf("NewCompletionFishCommand() = %v, want not nil", got)
			} else {
				if err := got.RunE(&c.Command{}, []string{}); err != nil {
					t.Errorf("Failed to run the completion fish command: %v", err)
				}
			}
		})
	}
...
func Test_runCompletionFish(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := runCompletionFish(tt.args.cmd, tt.args.output); (err != nil) != tt.wantErr {
				t.Errorf("runCompletionFish() error = %v, wantErr %v", err, tt.wantErr)
			}
			if tt.want != *tt.args.output {
				t.Errorf("runCompletionFish() = %v, want %v", *tt.args.output, tt.want)
			}
		})
	}
}
